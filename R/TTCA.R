...
#' following stimulation. A common observation in this type of data is that some genes respond with quick,
#' transient dynamics, while other genes change their expression slowly over time. The existing methods for the
#' detection of significant expression dynamics often fail when the expression dynamics show a large
#' heterogeneity, and often cannot cope with irregular and sparse measurements. The method proposed here is
#' specifically designed for the analysis of perturbation responses. It combines different scores to capture fast and
#' transient dynamics as well as slow expression changes, and deals with low replicate numbers and irregular
#' sampling times. The results are given in the form of tables linked to figures. These allow to quickly recognize
#' the relevance of detection, to identify possible false positives and to distinguish between changes in the early
#' and later expression. An extension of the method allows the analysis of the expression dynamics of functional
#' groups of genes, providing a quick overview of the cellular response. The performance of this package was
#' tested on microarray data derived from lung cancer cells stimulated with epidermal growth factor.
#' The related publication has been submitted to BMC Bioinformatics on 19.06.2016.
#' The manuscript title is "TTCA: An R package for the identification of differentially expressed genes in time course microarray data".
#'
#'
#' @param grp1        : Dataset with longitudinal sampled data (data.frame)
#' @param grp2        : Dataset with longitudinal sampled data (data.frame) for comparison
#' @param grp1.time   : Timepoints for Dataset 1 (vector like: c(0,0,0.5,1,2,4,6,8,12,12)
#' @param grp2.time   : Timepoints for Dataset 2 (vector like: c(0,0,0.5,3,2,4,6,8,12,12,24)
#' @param lambda      : Smoothingparameter in penalty term for fitting with quantil regression (Default: lambda=0.6 ). Find own parameter!
...
#' @param codetest    : Reduces the dataSet to 200 features for a quick pass of the program. (default: codetest=FALSE)
#' @param PeakMode    : Peakmode "norm" use variance between replicates, If you change to another character value, a normal hypothesis test will be conducted (default: PeakMode="norm")
#' @param timeInt     : Defines early, middle and late time period. Define the middle time period between 4 h and 12 h with timeInt<-c(4,12). (default: timeInt=NULL)
#' @param file        : Where the result folder should be created?   (default: file=getwd() )
#' @param MaxPics     : Limits the number of plots (default: MaxPics=10000)
#' @param Stimulus1   : With which term the gene shall be searched in Pubmed? Stimulus1="Insulin+like+growth+factor"   ( default: Stimulus1="")
#' @param Stimulus2   : With which term the gene shall be searched in Pubmed? Stimulus2="epidermal+growth+factor"      ( default: Stimulus2="")
...
#'
#' @return A table containing different significance values, rankings, p-values
#'
#' @details See publication "TTCA: An R package for the identification of differentially expressed genes in time course microarray data"
#'
#' @examples
#' \dontrun{
#' ##########################################
...
#######################################
## description of function values:
########################################
# grp1        : Dataset with longitudinal sampled data (data.frame)
# grp2        : Dataset with longitudinal sampled data (data.frame) for comparison
# grp1.time   : Timepoints for Dataset 1 (vector like: c(0,0,0.5,1,2,4,6,8,12,12)
# grp2.time   : Timepoints for Dataset 2 (vector like: c(0,0,0.5,3,2,4,6,8,12,12,24)
# lambda      : Smoothingparameter in Penalty term for spline fitting with quantil regression (Skalar: lambda=0.6 ). Find own parameter!
# annot       : annotation for pictures and result (Data.frame with 2 columns with ID and GeneName).
# pVal        : p-value for the local hypothesis test   (Skalar: default 0.05).
# codetest    : reduce the dataSet to 200 features for a quick pass of the program.
# PeakMode    : Peakmode "norm" use variance between replicates, If you change to another character value a normal hypothesis test will be conducted
########################################
...
if (Funtext==1){
print(paste0(format(Sys.time(), "%X"),": The work begins. Make a coffee or tea break. You deserve it ;D.                           "))
}
if (Funtext==2){
  print(paste0(format(Sys.time(), "%X"),": Perfect. TTCA works like crazy.                                                          "))
}
if (Funtext==3){
  print(paste0(format(Sys.time(), "%X"),": TTCA begins to enslave your computer. Let us burn the processor.                        "))
}
if (Funtext==4){
  print(paste0(format(Sys.time(), "%X"),": TCCA works. Time for day dreaming.                                                      "))
}
if (Funtext==5){
  print(paste0(format(Sys.time(), "%X"),": Absolute fantastic. You started TTCA!                                                   "))
}
if (Funtext==6){
  print(paste0(format(Sys.time(), "%X"),": TTCA cares now about your data. Do not worry, everything will be fine.                  "))
}
if (Funtext==7){
  print(paste0(format(Sys.time(), "%X"),": TTCA does something nice for you. Say something nice to a collegue.                     "))
}
if (Funtext==8){
  print(paste0(format(Sys.time(), "%X"),": Aye, aye sir. TTCA has leaved the harbour and comes hopefully back with                 "))
  print(paste0(format(Sys.time(), "%X"),": magnificient data. Pray to God that no pirates attack us with bugs.                     "))
}
if (Funtext==9){
  print(paste0(format(Sys.time(), "%X"),": Aye, aye sir. TTCA has leaved the harbour and comes hopefully back with                 "))
  print(paste0(format(Sys.time(), "%X"),": magnificient data. Pray to God that no pirates attack us with bugs.                     "))
...
if (Funtext==10){
  print(paste0(format(Sys.time(), "%X"),": TTCA has started to wash your data. Hopefully, we will obtain clear data.               "))
}
print(paste0("                                                                                                  "))
...
if(S=="GO"){
print(paste0(format(Sys.time(), "%X"),": GO-Modus.                                           "))
...
print(paste0(format(Sys.time(), "%X"),": ",beginSize-dim(grp1)[1] ," not annotated features have been removed (absolute ",round((1-dim(grp1)[1]/beginSize)*100,1)," % reduction)"  ))
...
print(paste0(format(Sys.time(), "%X"),": ",info-dim(grp1)[1] ," features have been removed because they have no positive intensity values (absolute ",round((1-dim(grp1)[1]/beginSize)*100,1)," % reduction)"  ))
rm(Z2)
}
rm(Z1,info)
########################################
...
if(codetest==TRUE){grp1.grp2<-grp1.grp2[sort(sample(1:nrow(grp1.grp2), 1100)), ]
                   ...
                   print(paste0(format(Sys.time(), "%X"),": Code testing modus. Works with 1100 features only                                       "))
                   }
################################################################################
########################################
## Start Mainpart
...
if(length(timeInt)!=2){ T1<-(max(grp1.grp2.time)-min(grp1.grp2.time))/3
                  ...
                  print(paste0(format(Sys.time(), "%X"),": Set early response period before ",round(timeInt[1],1),"h and late response period after ",round(timeInt[2],1),"h." ))}
###################
## classic
...
if (suppressWarnings(tryCatch( QueryCount(EUtilsSummary("TTCA")), error=function(e){-1}))==-1){print(paste0(format(Sys.time(), "%X"),": No access to webpage. Searching Pubmed for publication count was unsuccessful.           "))
  }else{
...
{
tt1<-as.numeric(unlist(strsplit( format(Sys.time(), "%X "),":")[1]))
...
print(paste0(format(Sys.time(), "%X"),": ",nrow(B)," Gene Ontology groups can be analyzed                                               "))
####
